...
Sécurité: veille à la validation d’entrée et gestion JWT/refresh
...
import { notifyDaoTeam } from "../services/daoNotificationService";
    ...
    try {
          ...
          if (changed.length > 0) {
            ...
            await notifyDaoTeam(
              updated,
              {
                ...
                data: { daoId: updated.id, changes: changed },
              },
              { actorId: req.user?.id },
            );
      ...
      try {
        ...
        if (changedKeys.size > 0 || !hasTaskChanges) {
          ...
          await notifyDaoTeam(updated, t, { actorId: req.user?.id });
        }
      } catch (_) {}
  ...
  async (req, res) => {
    try {
      ...
      try {
        ...
        await notifyDaoTeam(updated, notif, { actorId: req.user?.id });
      } catch (_) {}
