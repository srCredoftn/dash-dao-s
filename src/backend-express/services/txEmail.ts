...
export function sanitizeEmails(
  ...
  if (invalid.length > 0 && options.logInvalid !== false) {
    logger.warn("Invalid email recipients filtered", "MAIL", {
...
export async function sendEmail(
  ...
  logger.info(
    `Email désactivé (ignoré) | sujet: ${smtpSubject} | corps: ${preview}`,
    "MAIL",
  );
...
export const Templates = {
  ...
  dao: {
    ...
    updated(dao: Dao) {
      ...
      const body = [
        `Le DAO ${dao.objetDossier} a été mis �� jour :`,
        `Référence : ${dao.reference}`,
        `Objet : ${dao.objetDossier}`,
        `Autorité contractante : ${dao.autoriteContractante}`,
        `Date de dépôt : ${frDate(dao.dateDepot)}`,
  ...
  task: {
    ...
    commented(ctx: { dao: Dao; task: DaoTask; comment: string }) {
      ...
      const body = [
        ...
        ``,
        `Action : Nouveau commentaire ajout��`,
...
export async function emailAllUsers(
  ...
  const { valid, invalid } = sanitizeEmails(combined, {
